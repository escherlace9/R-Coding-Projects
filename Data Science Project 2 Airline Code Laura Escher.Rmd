...
author: "Laura Escher + Data Science Teammates"
date: "2025-12-04"
output:
  html_document:
    code_folding: hide
...
```{r}
airdata <- read.csv("sample airline data.csv file path") #referenced in the data files for this project
str(airdata)
...
For the last portion of EDA, we thought it would be a good idea to showcase the delays by origin and destination airport using airports with greater than or equal to 100 flights.
...
ggplot(worst_25, aes(x = reorder(ORIGIN_CITY_NAME, delay_rate), y = delay_rate)) +
  ...
  theme_minimal()
```
For Origin airports, Eagle, Colorado has the highest proportion of delayed flights at 47.5%.
...
For Destination airports, Provo, Utah has the highest proportion of delayed flights at 47.7%.
...
# Statistical Tests [Teammate explanation] & [Laura code]
...
There are significant differences in the average delay minutes across airlines (p < 2e−16).
Some airlines have much longer delays than others.

# Decision Tree [Teammate]

We read the data in.
```{r}
flightdata = read.csv("LAirlinedata.csv file path")
```
...
rf_model <- randomForest(isDelayed ~ ORIGIN_AIRPORT_ID + DAY_OF_MONTH + OP_UNIQUE_CARRIER + DISTANCE + AIR_TIME + region + ORIGIN_STATE_ABR + flightdata$DEP_HOUR,
                         data = flightdata,
                         ntree = 500,       
                         mtry = 2,          
                         importance = TRUE)
...
# Logistic Regression and Next Month Delay Prediction [Teammate]
...
Filter the data set to include only major U.S. airlines (AA, DL, UA, WN, AS, B6, HA, F9, NK) and restrict the data set to the top 10 airports reducing dimensionality.

```{r}
# Read monthly files
csv_files <- list.files(
  path = "teammate months file path",
...
then drop the columns that store delay minutes (containing NA mostly) by cancellation information, since they are not useful for initial prediction.
...
Split the cleaned data set into a training set (January–June flights) 
...
The matrix confusion is plotted and the interpretation is as follow: 
True negatives (TN): 25,492 flights that were on‑time and predicted on‑time.
False positives (FP): 1,254 flights that were on‑time but predicted delayed.
False negatives (FN): 10,843 flights that were delayed but predicted on‑time.
True positives (TP): 1,755 flights that were delayed and predicted delayed.
This shows the model misses many delays but rarely raises a false alarm.
...
- The model predict moderate risk, with 4 of 10 flights. 
- The prediction for the next month suggest that airlines as Atlanta, Denmark and Delta presents highest probability to arrive late.
- During the analysis, we observe that using only 3 predictor the ROC curve present a fair behavior(flat) with ROC value of 0.5. This result indicated that the probability of prediction is too low or moderate low, consequently we added more than 20 predictors and the new ROC increased to 0.7.   
- An important consideration must be starting from small data and according to the EDA analysis increase the data size.
