...
authors: "Laura Escher + Data Science Teammates"
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: hide
...
```{r}
spotify <- read.csv("C:/Users/esche/OneDrive - The George Washington University/Intro to Data Science/Data Science Project 1/FINAL VER. Data Science Project 1 Spotify Group 2/spotify_churn_dataset.csv")
...
We first opened up the file and examined the data using the str() function. We checked to see if there are any missing values in the data, which there are none. There are 8000 observations in this data set, and 12 variables. The variables are:
...
```
## Average income by country and churn

Now lets look at average income by country
US:83700  CA:63500  AU:65000  FR:47000  
DE:44000  UK:37000  IN:4500  PK:3500

Now the prediction should be that the higher, the income the lower the churn rate. Let's test that prediction.
...
## Churn to Active Ratio by Country Graph
...
## EDA for device_type and bar plot
...
## Listening time by device type churn status graph
...
Mean skip rates are almost identical (~0.30) across all subscription types with similar variability and balanced sample sizes. Visually and descriptively, no clear difference appears.
...
This tests whether mean skip rates differ significantly among the four subscription types.
...
Residuals show a mild S-shape (some tail deviation from normality), but with N = 8,000, the Central Limit Theorem makes ANOVA robust to this. No severe outliers are evident, so the normality assumption is acceptable.
...
Bartlett’s test yields p = 0.8, indicating no evidence of unequal variances across groups. The homogeneity-of-variance assumption holds for classical ANOVA.
...
# Spotify Project Conclusion:

We are of the opinion that this data is heavily artificial and lacks any correlations that might be reflective of world data. We do not know how this data was generated by the user on Kaggle. In terms of the future, it is important to look at the data before diving in to analysis. Obtaining the data from direct sources such as web scraping might be beneficial as well. Simply looking at the correlation matrix, any trend is heavily lacking. We further drew from outside sources, such as looking at the average income by country. We built upon this by using our SMART questions to have investigate any relationships in the data. After building our analysis, we found a lack of remarkable features in the data and similarly very little to no statistically significant relationships.

Although no significant relationships were found between individual categorical variables and churn, with p-values not reaching conventional levels of significance (all p > 0.2), exploring interactions between multiple categorical variables revealed some potential associations. Specifically, the interaction between country and gender when crossed with device type suggested relations worthy of further exploration. Statistical tests comparing means and variances across groups consistently showed similar results of no significant differences in these measures. These findings suggest that while single factors alone may not significantly predict churn, combinations of variables could provide more insight, highlighting the importance of considering interactions in churn analysis.

# Sources Cited

- Chaudry, W. (2025). Exploring the determinants of music popularity: A quantitative analysis of Spotify data (SSRN Working Paper No. 5126647). Social Science Research Network. https://ssrn.com/abstract=5126647

- Iacobucci, D. (2023). Analysis of Variance, Experimental Design, and Multivariate ANOVA (3rd ed.). Wiley

- nabihazahid. (2025). Spotify dataset for churn analysis [Data set]. Kaggle. https://www.kaggle.com/datasets/nabihazahid/spotify-dataset-for-churn-analysis/data

- Wikipedia. (n.d.). List of countries by average wage. Retrieved October 12, 2025, from https://en.wikipedia.org/wiki/List_of_countries_by_average_wage